...
(function () {
  ...
  function doKeepAlive() {
    ...
    fetch(CONFIG.keepAliveUrl, {
      ...
      .then(function (resp) {
        // If the session is invalid, backend should respond 401 + {ok:false}
        if (resp.status === 401) {
      ...
      .then(function (data) {
        ...
        // - markActivity(): resets local idle tracking
        // - closeModal(): hides warning UI
        // - ajaxCheckSession(): confirms server-side validity and closes modal if needed
        markActivity();
        lastServerValidAt = Date.now();
        setModalStatus('');
        closeModal();
